...
class WlDVL(WlDVLBase):
    ...
    def __init__(self, device, baudrate=115200, debug=False):
        try:
            self._serial = serial.Serial(device, baudrate, bytesize=8, parity='N', stopbits=1, timeout=None, xonxoff=False, rtscts=False, dsrdtr=False)
        except Exception as err:
            raise ValueError("Serial port not useable")
...
if __name__ == "__main__":
    # serial_test = serial.Serial(PORT, 115200, bytesize=8, parity='N', stopbits=1, timeout=None, xonxoff=False, rtscts=False, dsrdtr=False)
    test_dvl = WlDVL(PORT)
    while (True):
        print("Getting the following data: ")
        print(test_dvl.read())
        time.sleep(1)